...
namespace planner {
	...
	*/
	...
	private:
		...
		int type_code(std::string type) const;
		int compare_type(std::string typeA, std::string typeB) const;
	...
	int Planner::type_code(std::string type) const {
		int ans = -1;
	...
	std::string Planner::panel() {
		...
		std::string menu = 
			"\n-> Refresh__1, Next__2, Skip__3, Show list__4, Change note__5, Exit__0\n";
	...
	int Planner::compare_type(std::string typeA, std::string typeB) const {
		const int _DELAY = 0;
		const int DELAY = 1;
		...
		int typeCodeA = this->type_code(typeA);
		int typeCodeB = this->type_code(typeB);
	...
	int Planner::search() const {
		...
		else ans = min(ans_replaceable, ans_free);
		return ans;
	}
		...
		bool dec_flag = true; while (dec_flag == true) {
			...
			case 5:	//change something to note
				std::cin.get();
				{
					...
					int currentIndex = this->daylist_.pointer.number();
					this->daylist_[currentIndex].changeNote(theNote);
				}
